...
#        meanlevel = int(sys.argv[1][:2])
 #       stdlevel = int(sys.argv[1][-2:])
        ...
        gs1 = GridSpec(4, 2)
        gs1.update(left=0.03, right=0.98, wspace=0.1, hspace=0.6, bottom=0.15, top=0.92)
        plt.suptitle('Sensor response for ' + file[10:-6] + ' meters', fontsize=20)
...
# Frequency transform plot for X-Band
        ...
        plt.xlabel('Time, s')
        plt.title('Frequency transformation graph for raw radio wave signal, (b)')

        props = dict(boxstyle='round', facecolor='wheat', alpha=0.5)
        tex = 'Temperature: 19$^\circ$C\nTime: 21:00 - 22:30\nWind: 13 km/h'
        fr_tr_plt.text(0.84, 1.5, tex, transform=fr_tr_plt.transAxes, fontsize=14, verticalalignment='top', bbox=props)
